...
@section('content')
    ...
    <section id="news-preview" class="py-24 bg-white relative">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-end mb-12">
                <div class="max-w-2xl">
                    <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-4">Berita Terkini Lamongan</h2>
                    <p class="text-slate-500">Update seputar infrastruktur dan kabar terbaru dari Lamongan.</p>
                </div>
                <a href="{{ url('/news') }}" class="hidden md:inline-flex items-center text-blue-600 font-bold hover:text-blue-800 transition-colors">
                    Lihat Semua <i class="fas fa-arrow-right ml-2"></i>
                </a>
            </div>

            <div id="home-news-loading" class="flex justify-center items-center py-10">
                <div class="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-blue-600"></div>
            </div>

            <div id="home-news-grid" class="hidden grid grid-cols-1 md:grid-cols-3 gap-8">
                <!-- Fetch via JS -->
            </div>

            <div class="mt-10 text-center md:hidden">
                <a href="{{ url('/news') }}" class="inline-flex items-center px-6 py-3 bg-blue-50 text-blue-600 font-bold rounded-full hover:bg-blue-100 transition-colors">
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
            ...
            if (result.success && result.data && result.data.length > 0) {
                ...
                // Tampilkan hanya 3 berita terbaru
                ...
                latestNews.forEach(news => {
                    ...
                    card.innerHTML = `
                        ...
                        <div class="p-6 flex flex-col flex-grow">
                            ...
                            <h3 class="text-lg font-bold text-slate-900 mb-3 line-clamp-2 group-hover:text-blue-600 transition-colors">${news.title}</h3>
                        </div>
                    `;
                    grid.appendChild(card);
                });
            } else {
                grid.parentElement.classList.add('hidden'); // Sembunyikan section jika tidak ada berita
            }
        } catch (error) {
